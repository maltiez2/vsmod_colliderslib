...
public readonly struct LineSegmentCollider
{
    ...
    public bool RoughIntersect(CuboidAABBCollider collisionBox)
    {
        if (collisionBox.Min.X < Position.X && collisionBox.Max.X < (Position.X + Direction.X)) return false;
        if (collisionBox.Min.X > Position.X && collisionBox.Min.X > (Position.X + Direction.X)) return false;

        if (collisionBox.Max.Y < Position.Y && collisionBox.Max.Y < (Position.Y + Direction.Y)) return false;
        if (collisionBox.Min.Y > Position.Y && collisionBox.Min.Y > (Position.Y + Direction.Y)) return false;

        if (collisionBox.Max.Z < Position.Z && collisionBox.Max.Z < (Position.Z + Direction.Z)) return false;
        if (collisionBox.Min.Z > Position.Z && collisionBox.Min.Z > (Position.Z + Direction.Z)) return false;
    ...
    public Vector3d? IntersectCuboid(CuboidAABBCollider collisionBox, out double parameter)
    {
        double tMin = 0.0f;
        double tMax = 1.0f;
        ...
        if (!CheckAxisIntersection(Direction.X, Position.X, collisionBox.Min.X, collisionBox.Max.X, ref tMin, ref tMax)) return null;
        if (!CheckAxisIntersection(Direction.Y, Position.Y, collisionBox.Min.Y, collisionBox.Max.Y, ref tMin, ref tMax)) return null;
        if (!CheckAxisIntersection(Direction.Z, Position.Z, collisionBox.Min.Z, collisionBox.Max.Z, ref tMin, ref tMax)) return null;
    ...
    {
        bool isParallel = Math.Abs(dirComponent) < float.Epsilon;
        if (isParallel)
        {
            return startComponent >= minComponent && startComponent <= maxComponent;
        }
