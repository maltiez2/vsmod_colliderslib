    ...
    where TKey : notnull
{
    ...
    public void Clean()
    {
        ...
        try
        {
            Utils.LoggerUtil.Verbose(_api, this, $"({_loggedCacheName}) Starting clean up. Current world time: {TimeSpan.FromMilliseconds(currentTime)}\nSize: {_mapping.Count}\n'Get' count: {_getCountBetweenCleanUps}\n'Add' count: {_addCountBetweenCleanUps}");
            ...
            Utils.LoggerUtil.Verbose(_api, this, $"({_loggedCacheName}) Cleaned up '{keysToRemove.Count}' keys for '{entities.Count}' values.");
        }
        catch (Exception exception)
        {
            Utils.LoggerUtil.Error(_api, this, $"({_loggedCacheName}) Error on cache cleanup:\n{exception}");
        }
